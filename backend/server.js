...
// Import vnpayReturn controller để xử lý callback trực tiếp
const vnpayReturnController = require('./controllers/vnpayReturn.controller');

// VNPay return URL handler - route trực tiếp
app.get('/vnpay_return', (req, res, next) => {
  console.log('=== VNPAY RETURN REQUEST RECEIVED ===');
  ...
  res.setTimeout(10000, () => {
    if (!res.headersSent) {
      console.error('VNPay return timeout');
      res.redirect('http://localhost:3000/payment-status.html?status=error&message=Request timeout&code=TIMEOUT');
    }
  });

  try {
    console.log('=== DIRECT VNPAY RETURN HANDLER ===');
    console.log('Query params:', req.query);

    // Controller sẽ tự redirect
    await vnpayReturnController.handleVnpayReturn(req, res);
  } catch (error) {
    console.error('Direct VNPay return error:', error);
    if (!res.headersSent) {
      res.redirect('http://localhost:3000/payment-status.html?status=error&message=Processing error&code=SYSTEM_ERROR');
    }
  }
});
...
// Serve payment status page
app.get('/payment-status.html', (req, res) => {
  res.sendFile(path.join(__dirname, '../frontend/web/public/payment-status.html'));
});
